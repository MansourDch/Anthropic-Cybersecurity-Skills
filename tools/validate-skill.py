...
def parse_frontmatter(text):
    ...
    for line in block.split("\n"):
        ...
        key, colon, val = stripped.partition(":")
        if colon:
            if key and (key[0].isalnum() or key[0] == "_") and all(c.isalnum() or c in "_-" for c in key[1:]):
                val = val.strip()

                # Handle inline list: tags: [a, b, c]
                # Regex was: ^(\w[\w_-]*):\s*\[(.+)\]\s*$ (requires at least one char inside [])
