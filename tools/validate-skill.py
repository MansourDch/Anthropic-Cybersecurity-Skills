...
# Pre-compiled regexes for frontmatter parsing optimization
_INLINE_LIST_RE = re.compile(r"^(\w[\w_-]*):\s*\[(.+)\]\s*$")
_FOLDED_SCALAR_RE = re.compile(r"^(\w[\w_-]*):\s*>[-|]?\s*$")
_PLAIN_SCALAR_RE = re.compile(r"^(\w[\w_-]*):\s*(.*)$")
...
def parse_frontmatter(text):
    ...
    for line in block.split("\n"):
        ...
        m = _INLINE_LIST_RE.match(stripped)
        if m:
        ...
        m = _FOLDED_SCALAR_RE.match(stripped)
        if m:
            current_key = m.group(1)
            list_values = []
            in_folded = True
        ...
        m = _PLAIN_SCALAR_RE.match(stripped)
        if m:
            current_key = m.group(1)
