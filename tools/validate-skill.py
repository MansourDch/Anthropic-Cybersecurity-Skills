...
# Precompile regular expressions used in frontmatter parsing for better performance
INLINE_LIST_RE = re.compile(r"^(\w[\w_-]*):\s*\[(.+)\]\s*$")
FOLDED_SCALAR_START_RE = re.compile(r"^(\w[\w_-]*):\s*>[-|]?\s*$")
...
def parse_frontmatter(text):
    ...
    for line in block.split("\n"):
        ...
        m = FOLDED_SCALAR_START_RE.match(stripped)
        if m:
            current_key = m.group(1)
            list_values = []
            in_folded = True
