...
FRONTMATTER_INLINE_LIST_RE = re.compile(r"^(\w[\w_-]*):\s*\[(.+)\]\s*$")
FRONTMATTER_FOLDED_SCALAR_RE = re.compile(r"^(\w[\w_-]*):\s*>[-|]?\s*$")
FRONTMATTER_SCALAR_RE = re.compile(r"^(\w[\w_-]*):\s*(.*)$")
...
def parse_frontmatter(text):
    ...
    for line in block.split("\n"):
        ...
        m = FRONTMATTER_INLINE_LIST_RE.match(stripped)
        if m:
        ...
        m = FRONTMATTER_FOLDED_SCALAR_RE.match(stripped)
        if m:
            current_key = m.group(1)
            list_values = []
            in_folded = True
        ...
        m = FRONTMATTER_SCALAR_RE.match(stripped)
        if m:
            current_key = m.group(1)
